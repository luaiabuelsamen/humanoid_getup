...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--num-timesteps", type=int, default=None,
                   help="override ppo.num_timesteps from config")
    return p.parse_args()
...
def main() -> None:
    ...
    env      = HumanoidGetUp(env_cfg)
    eval_env = HumanoidGetUp(env_cfg)
    ...
    def save_checkpoint(current_step: int, _make_inference_fn, params) -> None:
    ...
    print(f"[train] devices={jax.devices()} -> {out_dir}", flush=True)
    ...
    with open(out_dir / "final.pkl", "wb") as f:
        pickle.dump({"params": params, "ppo_params": dict(ppo_params),
                     "net_hidden": list(policy_hidden)}, f)
    print(f"[train] wrote {out_dir/'final.pkl'}", flush=True)
