...
src/controls_playground/
  ...
  motor_dr.py           per-joint motor-DR env wrapper (gain/lag/saturation)
  scripts/
    train.py            training loop (cp-train)
    train_dr.py         training loop with motor DR enabled
    ...
    cost_eval.py        RL vs MPPI under MPPI's stage cost
    lqr.py / hybrid.py  LQR design at the policy's operating point + hybrid eval
    landscape.py        empirical state-landscape plot (cp-landscape)
    analysis.py         linearization probe and trajectory plot (cp-analysis)
...
`python -m controls_playground.scripts.train --config configs/train.yaml`.
...
DYLD_LIBRARY_PATH=$HOME/.local/share/uv/python/cpython-3.11.14-macos-aarch64-none/lib \
  ...
  -m controls_playground.scripts.viz --ckpt checkpoints/.../final.pkl
```

On Linux, just `cp-play --ckpt ...` works.

## MPC baseline + analysis plots
...
See `WRITEUP.md` Appendix B for the state-landscape interpretation.
